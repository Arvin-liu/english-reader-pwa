...
function populateVoiceSelect() {
  ...
  voices.forEach((voice) => {
    ...
    const serviceTag = typeof voice.localService === 'boolean'
...
function loadVoices() {
  ...
  voices = sortVoices(englishVoices);
  populateVoiceSelect();
  updateVoiceName();
}
...
function splitReadingUnits(line) {
  ...
  for (let i = 0; i < text.length; i += 1) {
    ...
    if (strong.has(ch)) {
      ...
      if (!isNumberingDot) pushBuffer();
      continue;
    }
    if (weak.has(ch)) {
      const current = buffer.trim();
      if (current.length >= 36) pushBuffer();
    }
  }
  ...
  paragraphs.forEach(paragraphText => {
    ...
    lines.forEach((line, lineIndex) => {
      ...
      units.forEach(unit => {
        ...
        const parts = unit.split(/([A-Za-z]+(?:'[A-Za-z]+)?)/g);
        parts.forEach(part => {
          if (/^[A-Za-z]+(?:'[A-Za-z]+)?$/.test(part)) {
            const word = document.createElement('span');
            word.textContent = part;
            word.className = 'word english-text';
...
function speakReadingUnit(unit) {
  if (!unit) return;
...
reloadVoices.addEventListener('click', () => {
  loadVoices();
  previewCurrentVoice();
});
...
(async function init() {
  ...
  try {
    const res = await fetch('dictionary.json?v=4');
